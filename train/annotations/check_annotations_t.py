...
def draw_polygons(image, labels, class_names):
    h, w = image.shape[:2]
    colors = [
        (0, 255, 0),
        (0, 0, 255),
    ...
    for cls, points in labels:
        ...
        pts_pixel = np.array([[int(x * w), int(y * h)] for x, y in points], dtype=np.int32)

        if len(pts_pixel) >= 3:
            cv2.fillPoly(overlay, [pts_pixel], color)
            cv2.polylines(image, [pts_pixel], True, color, 2)

        class_name = class_names[cls] if cls < len(class_names) else f'class_{cls}'
        x_min = pts_pixel[:, 0].min()
        y_min = pts_pixel[:, 1].min()
        label = f'{class_name}'
        (label_width, label_height), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
        cv2.rectangle(image, (x_min, y_min - label_height - 5), (x_min + label_width, y_min), color, -1)
        cv2.putText(image, label, (x_min, y_min - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
def main():
    parser = argparse.ArgumentParser(description='检查t数据集标注 (分割任务)')
    parser.add_argument('--split', default='train', help='数据集分割 (train/valid)')
    parser.add_argument('--max_images', type=int, default=20, help='最大检查图片数')
    parser.add_argument('--output', default='annotation_check_t', help='输出目录')
