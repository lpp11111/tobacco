...
def main():
    parser = argparse.ArgumentParser(description='检查small模型迁移能力')
    ...
    model_path = os.path.join(base_dir, 'models', 'best_small.pt')
    
    t_classes = ["grow tobacco", "disease tobacco", "others"]
    t_nc = 3
    
    small_classes = ['light_disease', 'healthy', 'severe_disease', 'moderate_disease']
    small_nc = 4
    ...
    print("\n类别映射规则 (small -> t):")
    print(f"  small(4类): {small_classes}")
    print(f"  t(3类): {t_classes}")
    print(f"  0({small_classes[0]}) -> 1({t_classes[1]})")
    print(f"  1({small_classes[1]}) -> 0({t_classes[0]})")
    print(f"  2({small_classes[2]}) -> 1({t_classes[1]})")
    print(f"  3({small_classes[3]}) -> 1({t_classes[1]})")
        ...
        for r in pred_results:
            if r.boxes is not None:
                for box in r.boxes:
                    ...
                    mapped_cls = class_mapping.get(cls, 2)
    ...
    csv_path = os.path.join(output_dir, 'transfer_results_small.csv')
