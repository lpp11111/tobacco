...
class TIFProcessor:
    # best_medium_finetuned.pt 为 2 类检测模型: 0=healthy, 1=disease
    DEFAULT_CLASS_NAMES = ['healthy', 'disease']
    ...
    def generate_vector(self):
        ...
        for i, r in enumerate(self.results):
            ...
            attrs = {
                ...
                'confidence': r['conf'],
                'class_id': r['class_id'],
                'class_name': r.get('class_name', f"class_{r['class_id']}")
            }
    ...
    def save_results_csv(self):
        ...
        headers = ['id', 'pixel_x1', 'pixel_y1', 'pixel_x2', 'pixel_y2',
                   'geo_x1', 'geo_y1', 'geo_x2', 'geo_y2', 'confidence', 'class_id', 'class_name']
        if self.use_ndvi:
            headers.append('ndvi')
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for i, r in enumerate(self.results):
                row = [
                    ...
                    r['conf'], r['class_id'], r.get('class_name', f"class_{r['class_id']}")
                ]
                if 'ndvi' in r and r['ndvi'] is not None:
                    row.append(r['ndvi'])
                writer.writerow(row)
    ...
    def print_class_stats(self):
        ...
        counter = Counter(r.get('class_name', f"class_{r['class_id']}") for r in self.results)
        print("\n类别统计:")
...
def main():
    ...
    parser.add_argument("--model", default="../models/best_medium_finetuned.pt", help="模型权重路径")
