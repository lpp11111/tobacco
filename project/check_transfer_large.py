...
def main():
    ...
    t_dataset_path = os.path.join(base_dir, 't')
    model_path = os.path.join(base_dir, 'train', 'large', 'large_train', 'weights', 'best.pt')
    ...
    print("\n类别映射规则:")
    print("  large: tobacco(0) -> grow tobacco(0)")
    ...
    for image_file in image_files:
        ...
        h, w = image.shape[:2]
        fourth_channel = np.mean(image, axis=2, keepdims=True).astype(image.dtype)
        input_image = np.concatenate([image, fourth_channel], axis=2)
        ...
        pred_results = model.predict(input_image, conf=args.conf_threshold, verbose=False)
        ...
        for r in pred_results:
            if r.boxes is not None:
                for box in r.boxes:
                    ...
                    x_center = (box.xywh[0][0] / input_image.shape[1]).item()
                    y_center = (box.xywh[0][1] / input_image.shape[0]).item()
                    width = (box.xywh[0][2] / input_image.shape[1]).item()
                    height = (box.xywh[0][3] / input_image.shape[0]).item()
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'transfer_check')
