...
def main():
    parser = argparse.ArgumentParser(description='检查medium模型迁移能力')
    ...
    model_path = os.path.join(base_dir, 'train', 'medium', 'medium_train', 'weights', 'best.pt')
    
    t_classes = ["grow tobacco", "disease tobacco", "others"]
    t_nc = 3
    
    class_mapping = {0: 1, 1: 0, 2: 1, 3: 1}
    medium_classes = ['light_disease', 'healthy', 'severe_disease', 'moderate_disease']
    ...
    csv_path = os.path.join(output_dir, 'transfer_results_medium.csv')
